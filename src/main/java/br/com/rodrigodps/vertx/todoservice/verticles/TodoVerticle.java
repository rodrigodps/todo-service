...
import br.com.rodrigodps.vertx.todoservice.service.JdbcTodoService;
import br.com.rodrigodps.vertx.todoservice.service.RedisTodoService;
import br.com.rodrigodps.vertx.todoservice.service.TodoService;
...
public class TodoVerticle extends AbstractVerticle {
    ...
    private void initData() {
        ...
        service.initData().setHandler(res -> {
            if (res.failed()) {
                LOGGER.severe("Persistence service is not running!");
    ...
    private void handleCreateTodo(RoutingContext context) {
        try {
            ...
            service.insert(todo).setHandler(resultHandler(context,
                    res -> {
                        if (res) {
                            context.response()
                                    .setStatusCode(201)
    ...
    private void handleGetTodo(RoutingContext context) {
        ...
        service.getCertain(todoID).setHandler(resultHandler(context,
                res -> {
                    if (!res.isPresent())
                        notFound(context);
                    else {
    ...
    private void handleGetAll(RoutingContext context) {
        service.getAll().setHandler(resultHandler(context,
                res -> {
                    if (res == null) {
                        serviceUnavailable(context);
                    } else {
    ...
    private void handleUpdateTodo(RoutingContext context) {
        try {
            ...
            service.update(todoID, newTodo).setHandler(resultHandler(context,
                    res -> {
                        if (res == null)
                            notFound(context);
                        else {
    ...
    private void handleDeleteAll(RoutingContext context) {
        service.deleteAll().setHandler(deleteResultHandler(context));
    }
